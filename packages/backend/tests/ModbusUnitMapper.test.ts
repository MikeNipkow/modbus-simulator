...
import {
  unitFromObject,
  unitToUnitProps,
} from "../src/mapper/ModbusUnitMapper.js";
  ...
  describe("unitToUnitProps", () => {
    ...
    describe("Error Handling", () => {
      test("should throw error for null ModbusUnit", () => {
        expect(() => unitToUnitProps(null as any)).toThrow(
          "Invalid ModbusUnit",
        );
      });

      test("should throw error for undefined ModbusUnit", () => {
        expect(() => unitToUnitProps(undefined as any)).toThrow(
          "Invalid ModbusUnit",
        );
      });
    });
      ...
      test("should fail for missing unitId", () => {
        ...
        if (!result.success) {
          expect(result.errors).toContain(
            "ModbusUnit unitId must be between 1 and 254",
          );
        }
      });
    ...
    describe("Invalid DataPoints", () => {
      ...
      test("should fail when DataPoint has invalid type", () => {
        ...
        if (!result.success) {
          ...
          expect(result.errors.some((e) => e.includes("valid type"))).toBe(
            true,
          );
        }
      });
      ...
      test("should fail when DataPoint has invalid address", () => {
        ...
        if (!result.success) {
          ...
          expect(result.errors.some((e) => e.includes("valid address"))).toBe(
            true,
          );
        }
      });
      ...
      test("should fail when DataPoint has invalid accessMode", () => {
        ...
        if (!result.success) {
          ...
          expect(
            result.errors.some((e) => e.includes("valid accessMode")),
          ).toBe(true);
        }
      });
        ...
        if (!result.success) {
          ...
          expect(
            result.errors.some((e) =>
              e.includes("defaultValue type does not match"),
            ),
          ).toBe(true);
        }
      });
      ...
      test("should fail when ASCII DataPoint is missing length", () => {
        ...
        if (!result.success) {
          ...
          expect(
            result.errors.some((e) =>
              e.includes("ASCII must have a valid length"),
            ),
          ).toBe(true);
        }
      });
      ...
      test("should fail when DataPoint has invalid simulation", () => {
        ...
        if (!result.success) {
          ...
          expect(
            result.errors.some((e) =>
              e.includes("enabled property of type boolean"),
            ),
          ).toBe(true);
        }
      });
      ...
      test("should fail when multiple DataPoints have validation errors", () => {
        ...
        if (!result.success) {
          ...
          expect(
            result.errors.filter((e) => e.includes("DataPoint error")).length,
          ).toBeGreaterThan(0);
        }
      });
    });
    ...
    describe("DataPoint Addition Conflicts", () => {
      ...
      test("should fail when DataPoints in different areas have incompatible types", () => {
        ...
        if (!result.success) {
          expect(
            result.errors.some((e) =>
              e.includes("cannot be in Coils or DiscreteInputs areas"),
            ),
          ).toBe(true);
        }
      });
      ...
      test("should preserve all properties in round-trip conversion", () => {
        ...
        if (result.success) {
          ...
          for (let i = 0; i < originalDPs.length; i++) {
            ...
            expect(newDPs[i]!.getAccessMode()).toBe(
              originalDPs[i]!.getAccessMode(),
            );
