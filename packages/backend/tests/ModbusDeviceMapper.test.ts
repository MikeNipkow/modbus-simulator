...
import {
  deviceFromObject,
  deviceToDeviceProps,
} from "../src/mapper/ModbusDeviceMapper.js";
  ...
  describe("deviceToDeviceProps", () => {
    ...
    describe("Error Handling", () => {
      test("should throw error for null ModbusDevice", () => {
        expect(() => deviceToDeviceProps(null as any)).toThrow(
          "Invalid ModbusDevice",
        );
      });

      test("should throw error for undefined ModbusDevice", () => {
        expect(() => deviceToDeviceProps(undefined as any)).toThrow(
          "Invalid ModbusDevice",
        );
      });
    });
  ...
  describe("deviceFromObject", () => {
    describe("Valid Object Conversions", () => {
      ...
      test("should create ModbusDevice with multiple units", () => {
        const obj = {
          ...
          port: 502,
          endian: Endian.BigEndian,
          modbusUnits: [{ unitId: 1 }, { unitId: 2 }, { unitId: 3 }],
        };
      ...
      test("should accept both BigEndian and LittleEndian", () => {
        ...
        expect(result2.success).toBe(true);
        if (result1.success)
          expect(result1.value.getEndian()).toBe(Endian.BigEndian);
        if (result2.success)
          expect(result2.value.getEndian()).toBe(Endian.LittleEndian);
      });
    });
    ...
    describe("Invalid Filename", () => {
      test("should fail for missing filename", () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.errors.some((e) => e.includes("valid filename"))).toBe(
            true,
          );
        }
      });
      ...
      test("should fail for invalid file extension", () => {
        ...
        expect(result.success).toBe(false);
        if (!result.success) {
          expect(result.errors.some((e) => e.includes("end with .json"))).toBe(
            true,
          );
        }
      });
      ...
      test("should fail for non-boolean enabled", () => {
        ...
        if (!result.success) {
          expect(result.errors).toContain(
            "ModbusDevice must have a valid enabled boolean",
          );
        }
      });
    });
    ...
    describe("Invalid Port", () => {
      test("should fail for missing port", () => {
        ...
        if (!result.success) {
          expect(result.errors).toContain(
            "ModbusDevice must have a valid port number",
          );
        }
      });
      ...
      test("should fail for port less than 1", () => {
        ...
        if (!result.success) {
          expect(result.errors).toContain(
            "ModbusDevice port must be between 1 and 65535",
          );
        }
      });
    ...
    describe("Invalid Endian", () => {
      test("should fail for missing endian", () => {
        ...
        if (!result.success) {
          expect(result.errors).toContain(
            "ModbusDevice must have a valid endian string",
          );
        }
      });
      ...
      test("should fail for invalid endian value", () => {
        ...
        if (!result.success) {
          expect(
            result.errors.some(
              (e) => e.includes("BigEndian") && e.includes("LittleEndian"),
            ),
          ).toBe(true);
        }
      });
    ...
    describe("Invalid Optional Properties", () => {
      test("should fail for non-string name", () => {
        ...
        if (!result.success) {
          expect(result.errors).toContain(
            "ModbusDevice must have a valid name string",
          );
        }
      });
      ...
      test("should fail for non-string vendor", () => {
        ...
        if (!result.success) {
          expect(result.errors).toContain(
            "ModbusDevice must have a valid vendor string",
          );
        }
      });
      ...
      test("should fail for non-string description", () => {
        ...
        if (!result.success) {
          expect(result.errors).toContain(
            "ModbusDevice must have a valid description string",
          );
        }
      });
    });
    ...
    describe("Invalid ModbusUnits", () => {
      test("should fail when unit has invalid unitId", () => {
        ...
        if (!result.success) {
          expect(
            result.errors.some((e) => e.includes("ModbusUnit error")),
          ).toBe(true);
          expect(
            result.errors.some((e) =>
              e.includes("unitId must be between 1 and 254"),
            ),
          ).toBe(true);
        }
      });
      ...
      test("should fail when unit contains invalid DataPoint", () => {
        ...
        if (!result.success) {
          ...
          expect(result.errors.some((e) => e.includes("DataPoint error"))).toBe(
            true,
          );
        }
      });
      ...
      test("should fail when multiple units have validation errors", () => {
        const obj = {
          ...
          port: 502,
          endian: Endian.BigEndian,
          modbusUnits: [{ unitId: 0 }, { unitId: 255 }],
        };
    ...
    describe("Round-trip Conversion", () => {
      test("should preserve all properties in round-trip conversion", () => {
        ...
        if (result.success) {
          ...
          expect(newDevice.getDescription()).toBe(
            originalDevice.getDescription(),
          );
