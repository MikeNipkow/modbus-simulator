...
export function dataPointFromObject(obj: any): ParseResult<DataPoint> {
    ...
    if (Array.isArray(obj.areas)) {
        ...
        for (const area of obj.areas) {
            ...
            if ((area === DataArea.DiscreteInput || area === DataArea.InputRegister) && obj.areas.length === 1 && obj.accessMode !== AccessMode.ReadOnly)
                errors.push('DataPoint in DiscreteInputs and InputRegisters can only have ReadOnly accessMode');
            else if (obj.type !== DataType.Bool && (area === DataArea.Coil || area === DataArea.DiscreteInput))
    ...
    if (obj.address === undefined || typeof obj.address !== 'number' || obj.address < 0 || obj.address > 65535)
        errors.push('DataPoint must have a valid address');

    // Check if accessMode is valid.
    if (obj.accessMode === undefined || !Object.values(AccessMode).includes(obj.accessMode))
    ...
    if (obj.simulation !== undefined && obj.type !== DataType.ASCII) {
        ...
        if (obj.simulation.enabled === undefined || typeof obj.simulation.enabled !== 'boolean') {
            errors.push('DataPoint simulation must have enabled property of type boolean');
            return { success : false, errors  : errors };
        }
        ...
        if (minValue < getMinValueForType(type)) {
            errors.push(`DataPoint simulation minValue is out of range for type ${type} in data point '${id}'`);
            return { success : false, errors  : errors };
        }
        if (maxValue > getMaxValueForType(type)) {
            errors.push(`DataPoint simulation maxValue is out of range for type ${type} in data point '${id}'`);
            return { success : false, errors  : errors };
        }
