...
export function deviceToDeviceProps(device: ModbusDevice): ModbusDeviceProps {
  ...
  if (!device) throw new Error("Invalid ModbusDevice");
 ...
 */
export function deviceFromObject(
  obj: any,
  filename: string,
): ParseResult<ModbusDevice> {
  // Collect errors.
  const errors: string[] = [];
  ...
  if (endian === undefined || typeof endian !== "string")
    ...
    errors.push(
      `ModbusDevice endian must be either '${Endian.BigEndian}' or '${Endian.LittleEndian}'`,
    );
  ...
  if (obj.modbusUnits && Array.isArray(obj.modbusUnits)) {
    ...
    for (const unitObj of obj.modbusUnits) {
      ...
      if (!unitResult.success) {
        errors.push(
          ...unitResult.errors.map((err) => `ModbusUnit error: ${err}`),
        );
        continue;
      }
      ...
      if (device.hasUnit(unit.getId())) {
        errors.push(
          `ModbusUnit with id '${unit.getId()}' already exists in ModbusDevice`,
        );
        continue;
      }
