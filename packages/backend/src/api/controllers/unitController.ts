...
export const getUnitFromRequest = (req: Request, res: Response): ModbusUnit | undefined => {
    ...
    if (!unit) {
        res.status(404).json({ error: `Unit with id ${unitId} not found in device ${device.getFilename()}` });
        return undefined;
    }
...
export const createUnitRoute = (req: Request, res: Response) => {
    ...
    if (device.hasUnit(unitId)) {
        res.status(409).json({ error: `Unit with id ${unitId} already exists in device ${device.getFilename()}` });
        return;
    }
