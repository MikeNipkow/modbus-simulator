...
import { unitToUnitDTO } from "./ModbusUnitDTOMapper.js";
 ...
 */
export function deviceToDeviceDTO(
  device: ModbusDevice,
  template: boolean = false,
): ModbusDeviceDTO {
  // Check if device is valid.
  if (!device) throw new Error("Invalid ModbusDevice object");

  // Map properties.
  const props = deviceToDeviceProps(device);
  const dto: ModbusDeviceDTO = {
    ...props,
    modbusUnits: device.getAllUnits().map((unit) => unitToUnitDTO(unit)),
    template: template,
 ...
 */
export function deviceDTOFromObject(
  obj: any,
  template: boolean = false,
): ParseResult<ModbusDeviceDTO> {
  // Collect errors.
  const errors: string[] = [];
