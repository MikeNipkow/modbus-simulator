...
export class ModbusUnit {
  ...
  constructor(props: ModbusUnitProps) {
    ...
    for (const dpProps of props.dataPoints) {
      ...
      if (!result.success)
        throw new Error(
          `Failed to add DataPoint with id '${dp.getId()}' to ModbusUnit ${this.unitId}: ${result.errors}`,
        );
    }
  }
  ...
  public addDataPoint(dataPoint: DataPoint): ParseResult<DataPoint> {
    ...
    for (const area of dataPoint.getAreas()) {
      if (
        ...
        dataPoint.getType() !== DataType.Bool
      ) {
        errors.push(
          `DataPoint of type ${dataPoint.getType()} cannot be added to area ${area}`,
        );
    ...
    if (this.hasDataPoint(dataPoint.getId())) {
      errors.push(
        `DataPoint with id '${dataPoint.getId()}' already exists in ModbusUnit ${this.unitId}`,
      );
      return { success: false, errors: errors };
    }
    ...
    for (const address of dataPoint.getOccupiedAddresses()) {
      for (const area of dataPoint.getAreas()) {
        if (this.hasDataPointAt(area, address)) {
          errors.push(
            `Address ${address} in area ${area} is already occupied in ModbusUnit ${this.unitId}`,
          );
          return { success: false, errors: errors };
        }
      }
  ...
  public deleteDataPoint(id: string): boolean {
    ...
    if (!(dataPoint instanceof DataPoint)) return false;
  ...
  public deleteDataPointFromArea(id: string, area: DataArea): boolean {
    ...
    if (dataPoint.getAreas().length <= 1) return false;
