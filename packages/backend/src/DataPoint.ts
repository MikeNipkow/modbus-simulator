...
import {
  getDataViewFromValue,
  getDefaultValueForType,
  getRegisterLengthFromType,
...
export class DataPoint {
  ...
  constructor(props: DataPointProps) {
    ...
    if (props.address < 0 || props.address > 65535)
      throw new Error("DataPoint must have a valid address");
    if (!props.accessMode) throw new Error("DataPoint must have an AccessMode");
    if (props.length !== undefined && props.length <= 0)
      throw new Error("DataPoint length must be greater than 0");
    if (props.type === DataType.ASCII && props.length === undefined)
      throw new Error("DataPoint of type String must have a length defined");

    // Check length for ASCII type.
    if (
      props.type === DataType.ASCII &&
      props.length !== undefined &&
      props.defaultValue !== undefined &&
      (props.defaultValue as string).length / 2 > props.length
    )
      throw new Error(
        "DataPoint defaultValue length exceeds defined length for String type",
      );
  ...
  public setValue(
    ...
    if (!this.hasWriteAccess() && !force) return false;
   ...
   */
  public getRegisterValue(
    offset: number = 0,
    endian: Endian = Endian.BigEndian,
  ): boolean | number {
    // Return value directly for Bool types.
    if (this.type === DataType.Bool) return this.value as boolean;
  ...
  public setRegisterValue(
    ...
    dataView.setUint16(index, value as number);
  ...
  public addDataArea(area: DataArea): void {
    ...
    if (
      ...
      throw new Error(
        `DataPoint of type ${this.type} cannot have area ${area}`,
      );

    // Add area if not already present.
    if (!this.hasDataArea(area)) this.areas.push(area);
  }
  ...
  public startSimulation(intervalMs: number = 1000): boolean {
    ...
    if (this.type === DataType.ASCII) return false;

    // Check if simulation is already running.
    if (this.isSimulationRunning()) return false;
  ...
  private generateRandomValue(): boolean | number | bigint | string {
    ...
    let random: number | bigint;
    if (!isBigIntType(this.type)) {
      const min = this.simulation.minValue as number;
      const max = this.simulation.maxValue as number;
      const range = max - min;

      // Generate random number within range.
      let randomNum = min + range * Math.random();

      // Round number, if datapoint type is not a float.
      if (
        this.getType() !== DataType.Float32 &&
        this.getType() !== DataType.Float64
      )
        randomNum = Math.round(randomNum);

      random = randomNum;
    } else {
      const min = BigInt(this.simulation.minValue);
      const max = BigInt(this.simulation.maxValue);
      const range = max - min;

      // Factor from 1-100.
      const randomFactor: bigint = BigInt(Math.round(Math.random() * 100));
      const randomOffset: bigint = (range * randomFactor) / 100n;

      random = min + randomOffset;
    }
  ...
  public getOccupiedAddresses(): number[] {
    ...
    for (let i = 0; i < this.getLength(); i++) addresses.push(this.address + i);
